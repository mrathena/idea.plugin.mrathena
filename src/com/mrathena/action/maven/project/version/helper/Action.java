package com.mrathena.action.maven.project.version.helper;
...
import com.mrathena.component.MrathenaApplicationComponent;
...
/**
 * @author mrathena on 2019/1/8 18:24
 */
public class Action extends AnAction {
	...
	private Document getPomDocument(String pomFilePath) {
		try (InputStream file = new FileInputStream(pomFilePath)) {
			Document document = MrathenaApplicationComponent.saxBuilder.build(file);
			return document;
		} catch (Exception ex) {
			return null;
		}
	...
	public void writeNewPomFile(Document document, String path) {
		...
		try (OutputStreamWriter osw = new OutputStreamWriter(new FileOutputStream(path), Charsets.UTF_8.name())) {
			MrathenaApplicationComponent.xmlOutputter.output(document, osw);
		} catch (Exception ex) {
		}
	}
